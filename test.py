...
# checkpoint = torch.load('C:/Users/ChiTron/Desktop/thesis/Fish-Flows-main/weight/20220626_Unet/model_best.pth.tar')
...
for i in range(len(img_paths)):
    ...
    mae += abs(np.sum(target)-pred_sum)
    rmse += (np.sum(target)-pred_sum)*(np.sum(target)-pred_sum)
    i += 1
